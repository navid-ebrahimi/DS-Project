...
namespace Proj1
{
    class HashTable
    {
        Hashtable drugs = new Hashtable();
        Hashtable diseases = new Hashtable();
        ...
        public bool DeleteDrug(string name)
        {
            ...
            {
                foreach(var alergy in (this.drugs[name] as Drug).getAlergies())
                {
        ...
        public void IncreaseByInflationRate(int percent)
        {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            sw.Start();
            string file1 = "datasets\\drugs1.txt";
            string file2 = "datasets\\diseases2.txt";
            string file3 = "datasets\\effects3.txt";
            string file4 = "datasets\\alergies4.txt";
            string drug_effects_lines = File.ReadAllText(file3);
            string disease_alergies_lines = File.ReadAllText(file4);
            string drugs_lines = File.ReadAllText(file1);
            string diseases_lines = File.ReadAllText(file2);
            string[] seperator = new string[] {"\r\n"};
            ...
            hashTable.LoadDrugs(drugs_lines);
            hashTable.LoadDiseases(diseases_lines);
            for(int i=0; i< druglinesLeng || i< diseaselinesLeng; i++)
            {
                if(i< druglinesLeng)
                    hashTable.LoadEffects(druglinesArr[i]);
                if(i< diseaselinesLeng) 
                    hashTable.LoadAlergies(diseaselinesArr[i]);
            }
            System.Console.WriteLine(sw.Elapsed.TotalSeconds);
            sw.Restart();
            hashTable.WriteToFiles(file1, file2, file3, file4);
            System.Console.WriteLine(sw.Elapsed.TotalSeconds);
            sw.Stop();
        }
    }
